...
func (s *server) cycleWindows() {
	// Collect all non-panel, non-overlay windows on current desktop
	...
	for i, w := range windows {
		if w == s.activeXdg || w == s.activeXway {
			currentIdx = i
			break
		}
	}
...
func (s *server) cycleWindowsBackward() {
	// Collect all non-panel, non-overlay windows on current desktop
	var windows []interface{}
	for _, v := range s.xdgViews {
		if v.mapped && v.onDesk(s.currentDesk) {
	...
	prevIdx := (currentIdx - 1 + len(windows)) % len(windows)
	switch v := windows[prevIdx].(type) {
	case *xdgView:
		s.focusXdgView(v)
	case *xwayView:
		s.focusXwayView(v)
